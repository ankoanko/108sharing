...
post.calendars.create(day: Date.current + 30.day, reserved: true)
post.calendars.create(day: Date.current + 31.day, reserved: true)
post.calendars.create(day: Date.current + 32.day, daily_price: 5000)
post.calendars.create(day: Date.current + 33.day, daily_price: 5000)
post.calendars.create(day: Date.current + 34.day, daily_price: 5000)
post.calendars.create(day: Date.current + 35.day, daily_price: 5000)
post.calendars.create(day: Date.current + 36.day, daily_price: 5000)
...
reservation.conversation.messages.create(sender_id: host.id, body: "ついてます")
reservation.conversation.messages.create(sender: guest, body: "ありがとうございます！！！")
reservation.conversation.messages.create(sender_id: host.id, body: "当日はお気をつけておこしくださいね")
...
10.times do |i|
  ...
  post.reservations.create!(
    ...
    cancel_fee: 0,
    initial_message: "さっそく具体的なチェック時間を相談させてください！"
  )
end
...
# identification2 = User.first.identifications.new
# identification2.image.attach(io: File.open(Rails.root.join("spec/fixtures/images/", "license2.jpg")), filename: 'license2.jpg')
# identification2.save!


# conversation = reservation Conversation.between(User.find([user.id, user2.id]))


# guest_review1 = reservation.reviews.create(reviewable: reservation.post.user, rating: 4, body: "とてもきれいな部屋で快適でした、オーナーさんも親切でよかったです", reviewer: reservation.user)
# host_review1 = reservation.reviews.create(reviewable: reservation.user, rating: 4, body: "きれいに使っていただけました！", reviewer_id: reservation.post.user.id)

# guest_review1.review_replies.create(body: "またおこしください！", user_id: reservation.post.user.id)

# res.reviews.create(reviewable: res.post, reviewer: res.user, rating: 3, body: "aaaaaaa")
# Review.last.review_replies.create(body: "またおこしください！", user_id: res.post.user.id)

# guest_review2 = reservation.reviews.build(reviewable: reservation.post.user, rating: 4, body: "とてもきれいな部屋で快適でした、オーナーさんも親切でよかったです", reviewer: user2)
# guest_review3 = reservation.reviews.build(reviewable: reservation.post.user, rating: 4, body: "とてもきれいな部屋で快適でした、オーナーさんも親切でよかったです", reviewer: user2)


# post.reviews.create(reviewer: user2, rating: 4, body: "とてもきれいな部屋で快適でした")
# post.reviews.create(reviewer: user, rating: 3, body: "狭かったので、評価3にしました")
